...
import { IoArrowDownOutline, IoArrowUpOutline, IoCloseSharp } from 'react-icons/io5';
...
function TickerDisplayComponent({ ticker, showBg }: { ticker: Ticker; showBg?: boolean }) {
  const { disPlayDiff, diff } = tickerToDisplayDiff(ticker);

  return (
    <span
      className={`${diff < 0 ? 'text-sellRed ' : diff > 0 ? 'text-buyGreen ' : 'text-white'}  text-xs flex items-center  rounded-md ml-2  py-0.5`}
    >
      {' '}
...
function SymbolSelector(props: { setSymbol: (symbolName: string) => void; symbolName: string; mouseLeave: () => void }) {
  ...
  useEffect(() => {
    if (!allTickers) return;
      ...
      setSymbolList(
        <>
          {allTickers
            ?.filter((t) => t.symbol.toLowerCase().includes(searchValue?.toLocaleLowerCase() || ''))
            .map((t) => {
...
function ChartHeader() {
  ...
  return (
    <div className='flex items-center  text-white text-sm'>
      {/* icon */}
      <div
        className={`flex relative items-center flex-shrink-0 ${hoverSymbol ? 'cursor-pointer bg-symbolHover rounded-lg' : ''} 
      ...
      >
        ...
        <IoMdArrowDropdown color={!hoverSymbol ? '#566069' : '#FFFFFF'} size={20} className='ml-2' />
        {hoverSymbol && (
          <SymbolSelector
            mouseLeave={() => {
              setHoverSymbol(false);
